 ...
 *
 * CONCEITO: TRATAMENTO DE EXCEÇÕES
 * - try-catch para exceções checked (ContaBancariaException)
 * - Tratamento específico por tipo (SaldoInsuficienteException)
 * - Captura também RuntimeException (erros inesperados)
 *
 * CONCEITO: POLIMORFISMO
 * - Trabalha com ContaBancaria (interface)
 * - Cria diferentes tipos (ContaCorrente, ContaPoupanca)
 * - Operações funcionam para qualquer tipo
 *
 * CONCEITO: try-with-resources
 * - Scanner é AutoCloseable
 * - Fechado automaticamente no fim do bloco try
 */
public class MenuBanco {
    ...
    static void criarContaPoupanca(Scanner sc) {
        ...
        String titular = UtilsIO.lerStringNaoVazia(sc, "Titular: ");
        double saldoInicial = UtilsIO.lerDouble(sc, "Saldo inicial: ");
        int limiteLevantamentos = UtilsIO.lerInt(sc, "Limite de levantamentos mensais: ");
        
        ContaPoupanca conta = new ContaPoupanca(numero, titular, saldoInicial, limiteLevantamentos);
        banco.adicionarConta(conta);
